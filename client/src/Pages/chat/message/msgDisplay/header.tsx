...
export default function ChatHeader(props: any) {
  ...
  const chatCmd = async (cmd: string) => {
    ...
    if (errorLog !== undefined) {
      if (errorLog.substring(0, 5) === "Error") setAlert(errorLog, "error");
      else setAlert(errorLog, "success");
    }
  };
  ...
  if (chan?.type === "dm") {
    ...
    else dmUser = getProfile(props.state.userList, chan.members[0].id);
    profileLink = "/profile?userId=" + dmUser?.id.toString();
    if (dmUser) title = dmUser.login.valueOf();
  } else title = chan?.title;
  return (
    <div
      ...
      <div style={{ textAlign: "left", marginLeft: "5px", display: "flex" }}>
        ...
        <div style={{ marginLeft: "10px" }}>
          {dmUser === undefined || dmUser === null ? (
            ...
            >
              {getProfile(props.state.userList, dmUser.id)?.username}
            </span>
          )}{" "}
        </div>
        {dmUser !== undefined &&
        ...
        isBlocked(props.state.actualUser.user, dmUser) ? (
          <i style={{ fontSize: "10px" }}>[blocked]</i>
        ) : null}
        {chan?.ownerId === props.state.actualUser.user.id ? (
          <Tooltip title="Owner">
            <Icon icon="mdi:shield-crown" color="gray" inline={true} />
          </Tooltip>
        ) : null}
        {isAdmin(props.state.actualUser.user.id, chan) ? (
      ...
      <div>
        ...
        isBlocked(props.state.actualUser.user, dmUser) ? (
          <Tooltip title="Unblock user">
            <LockOpenIcon
              onClick={(event) => chatCmd("/unblock " + title.valueOf())}
              sx={{ cursor: "pointer", color: "grey", marginLeft: "45px" }}
            />
          </Tooltip>
        ) : null}
        {chan?.type === "public" || chan?.type === "private" ? (
          <Tooltip
            title="Leave channel"
            sx={{ cursor: "pointer", color: "grey", marginLeft: "45px" }}
          >
            <ExitToAppIcon onClick={(event) => chatCmd("/leave")} />
          </Tooltip>
        ) : null}
      </div>
    </div>
